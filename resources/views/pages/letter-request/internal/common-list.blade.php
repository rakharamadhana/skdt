            ...
            <nav class="level">
                ...
                <div class="level-right">
                    <div class="level-item">
                        <button class="button is-small is-info button-modal modal-button" data-target="#modal-manage-item" type="button" onclick="loadContent('letter-request/create', 'modal-manage-item-content')">
                            <span class="icon">
...
<script>
    var primary_table = $('#primary_table').DataTable({
        ...
        pageLength: 25,
        ajax: {
            url: base_url + 'letter-request/dt',
            type: 'GET',
                ...
                render: function(data) {
                    ...
                    if(data.status == 1){
                        html += `Perlu Revisi: <br>`
                            + data.notes +
                            `<br><button class="button is-small is-table is-info button-modal modal-button" data-target="#modal-manage-item" type="button" onclick="loadContent('letter-request/` + data.id + `/edit', 'modal-manage-item-content')">` +
                            `    <span>Revisi</span>` +
                            `</button>`;
                    }
                    if(data.status == 2){
    ...
    async function deleteAction(element) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "DELETE",
                    url: base_url + 'letter-request/' + item.attr('data-id'),
                    async: false,
