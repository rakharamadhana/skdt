...
<script>
    var primary_table = $('#primary_table').DataTable({
        ...
        pageLength: 25,
        ajax: {
            url: base_url + 'submission-approval/dt',
            type: 'GET',
            ...
            },
            {
                ...
                render: function(data) {
                    ...
                    if(data.status == 0 || data.status == 10){
                        html += `<button class="button is-small is-table is-info button-modal modal-button" data-target="#modal-manage-item" type="button" onclick="loadContent('submission-approval/` + data.id + `/edit', 'modal-manage-item-content')">` +
                            `    <span>Tindakan</span>` +
                            `</button>`;
                    }
                    if(data.status == 1){
                    ...
                    if(data.status == 2){
                        ...
                        `    Download LOA` +
                        `</a><br>`;
                        html += `<button class="button is-small is-table is-black button-modal modal-button" data-target="#modal-manage-item" type="button" onclick="loadContent('submission-approval/` + data.id + `/edit', 'modal-manage-item-content')">` +
    ...
    async function deleteAction(element) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "DELETE",
                    url: base_url + 'submission-approval/' + item.attr('data-id'),
                    async: false,
